...
my @dirs = ('/Users/creatingmyth/work/codes/quick-cocos2d-x_robin/framework');
my @tolua_dirs = ('/Users/creatingmyth/work/codes/quick-cocos2d-x_robin/lib/luabinding','/Users/creatingmyth/work/codes/quick-cocos2d-x_robin/bin/project/Classes/luabinding');
...
sub parse_tolua {
    ...
    for my $f(@files){
        ...
        while($c =~ m{[^a-zA-Z0-9 ]class\s+(\w+).*?\{(.*?)[^@* ]\}[^\.]}gs){
            my $class_name = $1;
            my $body = $2;
            my @functions = ();
            # print "clalss: ",$class_name, $body , "\n";
            while($body =~ m{\s*(.*?)\s*[\*]*\s*(\w+)\((.*?)\)\s*;}g){
                my $prefix = $1;
                if ($prefix =~ m{\/\/}){
                    next;
                }
